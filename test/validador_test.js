...
describe(" Validadores", function() {
    ...
    it("Deberia devolver true cuando quiero validar el pago del salario por comision un dia viernes y es semana par" , function() {
        ...
        let fechaActual = new Date("April 5 2019");
        expect(validador.validadorSalarioPorComision(fechaActual)).equal(true);
    });

    it("Deberia devolver true cuando quiero validar el pago del salario por comision un dia viernes y es semana par" , function() {
        let validador = new ValidadorSalarioPorComision();
        let fechaActual = new Date("April 12 2019");
        expect(validador.validadorSalarioPorComision(fechaActual)).equal(false);
    });
});